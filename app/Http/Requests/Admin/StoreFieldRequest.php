...
class StoreFieldRequest extends FormRequest
{
    ...
    protected function prepareForValidation(): void
    {
        ...
        // Inherit is_filterable from the catalog default when not explicitly set.
        if (! $this->has('is_filterable') && $this->filled('type')) {
            $def = FieldTypeRegistry::get($this->string('type'));
            if ($def !== null) {
                $this->merge(['is_filterable' => $def['is_filterable'] ?? false]);
            }
        }
    }
    ...
    public function rules(): array
    {
        ...
        return [
            ...
            'key'               => ['required', 'string', 'max:100', 'regex:/^[a-z][a-z0-9_]*$/',
                Rule::unique('fields')->where('field_group_id', $groupId)],
