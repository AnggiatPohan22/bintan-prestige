...
>
    ...
    <div class="flex shrink-0 border-b border-slate-800">
        <button
            ...
            :class="activeTab === 'insert' ? 'border-b-2 border-amber-500 text-white' : 'text-slate-400 hover:text-slate-300'"
        ...
        <button
            ...
            :class="activeTab === 'patterns' ? 'border-b-2 border-amber-500 text-white' : 'text-slate-400 hover:text-slate-300'"
            class="flex flex-1 items-center justify-center gap-1.5 py-3 text-xs font-semibold transition-colors"
        >
        ...
        <button
            ...
            :class="activeTab === 'tree' ? 'border-b-2 border-amber-500 text-white' : 'text-slate-400 hover:text-slate-300'"
            class="flex flex-1 items-center justify-center gap-1.5 py-3 text-xs font-semibold transition-colors"
        >
    ...
    <div x-show="activeTab === 'insert'" class="builder-pane-scroll flex-1 overflow-y-auto" x-cloak>
        ...
        <div class="border-b border-slate-800 px-3 py-2 text-xs text-slate-400">
            ...
            <template x-if="selectedNode()">
                <span>
                    ...
                    <button
                        ...
                        class="ml-1 text-slate-400 hover:text-slate-300"
        ...
        @foreach($catOrder as $cat)
            @if($categorized->has($cat))
                <div class="mb-1">
                    <p class="px-3 pb-1 pt-3 text-xs font-semibold uppercase tracking-widest text-slate-400">
    ...
    <div x-show="activeTab === 'patterns'" class="builder-pane-scroll flex-1 overflow-y-auto" x-cloak>
        ...
        <div x-show="patternsLoading" class="flex items-center justify-center gap-2 p-6 text-xs text-slate-400">
        ...
        <div x-show="!patternsLoading && patterns.length === 0" class="p-6 text-center text-xs text-slate-400" x-cloak>
            <i class="fa-solid fa-shapes mb-3 block text-3xl text-slate-300"></i>
                ...
                <article class="rounded-lg border border-slate-700 bg-slate-800/60 p-3">
                    <div class="flex items-start gap-2">
                        ...
                        <div class="min-w-0 flex-1">
                            ...
                            <p class="mt-0.5 text-[10px] uppercase tracking-wide text-slate-400" x-text="pattern.category || pattern.block_type"></p>
                        </div>
                        <button type="button"
                    ...
                    <button type="button"
                            ...
                            class="mt-3 w-full rounded-md bg-amber-500 px-2 py-1.5 text-xs font-semibold text-slate-100 transition hover:bg-amber-400">
    ...
    <div x-show="activeTab === 'tree'" class="builder-pane-scroll flex-1 overflow-y-auto" x-cloak>
        ...
        <div x-show="tree.length === 0" class="p-4 text-center text-xs text-slate-400" x-cloak>
            No blocks yet.<br>
            Switch to <strong class="text-slate-400">Add Block</strong> to start.
        </div>

        {{-- How-to hint for nesting --}}
        <div x-show="tree.length > 0" class="border-b border-slate-800 px-3 py-2 text-[11px] leading-snug text-slate-400" x-cloak>
        ...
        <ul class="py-1">
            <template x-for="row in flatList()" :key="row.node._cid">
                ...
                >
                    ...
                    {{-- Grip affordance --}}
                    <i class="fa-solid fa-grip-vertical w-2.5 shrink-0 text-center text-[10px] text-slate-400 group-hover:text-slate-400"></i>
                    ...
                    <i
                        ...
                        :class="[
                            ...
                            !row.node.is_visible ? 'text-slate-300' : (isContainer(row.node) ? 'text-amber-500/70' : 'text-slate-400'),
                        ]"
                    ></i>
                    ...
                    <span
                        ...
                        x-cloak
                        class="pointer-events-none shrink-0 rounded border border-slate-600 px-1 py-0.5 text-[9px] font-medium text-slate-400"
                    ...
                    <span
                        ...
                        class="pointer-events-none shrink-0 rounded bg-amber-500 px-1.5 py-0.5 text-[10px] font-semibold text-slate-100"
                    ...
                    <div class="flex shrink-0 items-center gap-0.5 opacity-0 transition-opacity group-hover:opacity-100" x-show="dragCid === null">
                        <button type="button" x-on:click.stop="toggleVisible(row.node)"
                                class="flex h-5 w-5 items-center justify-center rounded text-slate-400 transition-colors hover:text-white"
                        ...
                        <button type="button" x-on:click.stop="if(confirm('Remove this block?')) removeBlock(row.node._cid)"
                                class="flex h-5 w-5 items-center justify-center rounded text-slate-400 transition-colors hover:text-red-400" title="Delete block">
                            <i class="fa-solid fa-trash-can text-xs"></i>
                        </button>
                    </div>
                </li>
