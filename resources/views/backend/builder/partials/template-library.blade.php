    ...
    >
        <header class="flex shrink-0 items-center justify-between border-b border-slate-800 px-5 py-4">
            <div>
                ...
                <p class="mt-0.5 text-xs text-slate-500">Layout shells and reusable full-page designs</p>
            </div>
            <button type="button" x-on:click="templateModalOpen = false" class="text-slate-500 hover:text-white" title="Close">
            ...
            <section>
                <div class="mb-3 flex items-center justify-between">
                    <div>
                        ...
                        <p class="mt-1 text-[11px] text-slate-600">Changes the frontend layout without replacing blocks.</p>
                    </div>
                ...
                <div class="grid gap-3 sm:grid-cols-3">
                    <template x-for="layout in layoutTemplates" :key="layout.id">
                        <article class="rounded-lg border p-3" :class="Number(currentTemplateId) === Number(layout.id) ? 'border-amber-500 bg-amber-950/20' : 'border-slate-700 bg-slate-800/50'">
                            ...
                            <p class="mt-1 line-clamp-2 text-[11px] leading-snug text-slate-500" x-text="layout.description || layout.blade_file"></p>
            ...
            <section class="mt-7 border-t border-slate-800 pt-5">
                <div class="mb-3 flex flex-wrap items-end justify-between gap-3">
                    <div>
                        ...
                        <p class="mt-1 text-[11px] text-slate-600">Versioned full-page block trees; applying replaces the current canvas after confirmation.</p>
                    </div>
                ...
                <div x-show="templatesLoading" class="p-6 text-center text-xs text-slate-500">
                    <i class="fa-solid fa-spinner fa-spin mr-1"></i>Loading templatesâ€¦
                </div>
                <div x-show="!templatesLoading && builderTemplates.length === 0" class="rounded-lg border border-dashed border-slate-700 p-6 text-center text-xs text-slate-500">
                        ...
                        <article class="rounded-lg border border-slate-700 bg-slate-800/50 p-3">
                            <div class="flex items-start gap-2">
                                ...
                                <div class="min-w-0 flex-1">
                                    ...
                                    <p class="mt-0.5 text-[10px] uppercase tracking-wide text-slate-500" x-text="(template.category || 'Page') + ' Â· v' + template.schema_version"></p>
                                </div>
                                <button type="button" x-on:click="if(confirm('Delete this saved template? Existing pages remain unchanged.')) deleteBuilderTemplate(template)"
                                        class="text-slate-600 hover:text-red-400" title="Delete template">
                                    <i class="fa-solid fa-trash-can text-xs"></i>
                                </button>
                            </div>
                            <p x-show="template.description" x-text="template.description" class="mt-2 line-clamp-2 text-[11px] text-slate-500"></p>
                            <p class="mt-2 text-[10px] text-slate-600" x-text="'Layout: ' + template.base_template_name"></p>
        ...
        <div x-show="templateFormOpen" class="absolute inset-0 z-10 flex items-center justify-center bg-black/70 p-4" x-cloak>
            <form x-on:submit.prevent="saveBuilderTemplate()" class="w-full max-w-md space-y-3 rounded-xl border border-slate-700 bg-slate-900 p-5 shadow-2xl">
                ...
                <div class="flex gap-2 pt-2">
                    <button type="submit" :disabled="isSavingTemplate || !templateDraft.name.trim()" class="flex-1 rounded-md bg-amber-500 py-2 text-xs font-semibold text-slate-950 disabled:opacity-40">
