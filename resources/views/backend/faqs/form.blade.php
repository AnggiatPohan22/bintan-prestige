...
<div class="rounded-xl bg-white p-6 shadow">
    <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><h1 class="text-2xl font-bold text-slate-800">{{ $title }}</h1><p class="mt-1 text-sm text-slate-500">Manage reusable frontend FAQ content.</p></div><a href="{{ route('admin.faqs.index') }}" class="btn-secondary">Back</a></div>
    <form method="POST" action="{{ $action }}" class="space-y-5">
        ...
        <div class="flex items-center gap-3 border-t pt-5"><button type="submit" class="btn-primary">Save FAQ</button><a href="{{ route('admin.faqs.index') }}" class="btn-secondary">Cancel</a></div>
    </form>
</div>
