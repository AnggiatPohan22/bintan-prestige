...
<div class="rounded-xl bg-white p-6 shadow">
    ...
    <form method="POST" action="{{ $action }}" class="space-y-5">
        ...
        <div>
            ...
            <input type="text" name="question" value="{{ old('question', $faq->question) }}" class="{{ $inputClass }}">
            @error('question') <p class="form-error">{{ $message }}</p> @enderror
        </div>

        <div>
            <label class="form-label">Answer</label>
            <textarea name="answer" rows="6" class="{{ $inputClass }}">{{ old('answer', $faq->answer) }}</textarea>
            @error('answer') <p class="form-error">{{ $message }}</p> @enderror
        </div>

        <div>
            <label class="form-label">Sort order</label>
            <input type="number" name="sort_order" min="0" value="{{ old('sort_order', $faq->sort_order ?? 0) }}" class="{{ $inputClass }}">
