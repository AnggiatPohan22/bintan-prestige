...
<div class="admin-page">
    ...
    <div class="mb-6 flex flex-wrap items-start justify-between gap-4">
        <div>
            <h1 class="text-lg font-extrabold text-slate-100">Themes</h1>
        ...
        <div class="flex flex-wrap items-center gap-2" x-data="{ importOpen: false }">
            ...
            <div
                ...
                x-transition
            >
                <div class="mt-3 rounded-2xl border border-slate-700 bg-slate-800 p-5">
                    <h2 class="mb-1 text-sm font-bold text-slate-300">Import Theme from ZIP</h2>
                    <p class="mb-4 text-xs text-slate-400">
                        Upload a <code class="rounded bg-slate-700 px-1 border border-slate-600">.zip</code>
                    ...
                    >
                        ...
                        <div class="flex-1 min-w-48">
                            <label class="mb-1 block text-xs font-semibold text-slate-400 uppercase tracking-wide">
                                Theme ZIP File
                            </label>
                            <input
                                type="file"
    ...
    @if($themes->isEmpty())
        ...
        <div class="admin-empty-state py-12">
            ...
            <p class="font-semibold text-slate-300">No themes registered yet</p>
            <p class="mt-1 max-w-xs text-center text-sm text-slate-400">
                Place a <code class="rounded bg-slate-800 px-1 py-0.5 text-xs">theme.json</code> file
                inside each subdirectory of <code class="rounded bg-slate-800 px-1 py-0.5 text-xs">themes/</code>,
                then click <strong>Scan for Themes</strong>.
            </p>
        </div>

        {{-- Getting started guide --}}
        <div class="mt-8 rounded-2xl border border-slate-700 bg-slate-800 p-6">
            <h2 class="mb-3 text-sm font-bold text-slate-300">Getting Started</h2>
            <ol class="space-y-2 text-sm text-slate-400">
                <li class="flex gap-2">
                    ...
                    Create a subdirectory under <code class="rounded bg-slate-700 px-1 py-0.5 text-xs border border-slate-600">themes/your-theme-slug/</code>
                </li>
                <li class="flex gap-2">
                    ...
                    Add a <code class="rounded bg-slate-700 px-1 py-0.5 text-xs border border-slate-600">theme.json</code> manifest with <code class="rounded bg-slate-700 px-1 py-0.5 text-xs border border-slate-600">name</code>, <code class="rounded bg-slate-700 px-1 py-0.5 text-xs border border-slate-600">slug</code>, and optionally <code class="rounded bg-slate-700 px-1 py-0.5 text-xs border border-slate-600">widget_areas</code> and <code class="rounded bg-slate-700 px-1 py-0.5 text-xs border border-slate-600">customization_schema</code>
                </li>
                <li class="flex gap-2">
    ...
    @else
        ...
        <div class="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
            @foreach($themes as $theme)
                ...
                <div class="flex flex-col rounded-2xl border {{ $theme->is_active ? 'border-violet-500 ring-1 ring-violet-500/30' : 'border-slate-700' }} bg-slate-800">

                    {{-- Screenshot / placeholder --}}
                    <div class="relative flex h-36 items-center justify-center rounded-t-2xl overflow-hidden {{ $theme->is_active ? 'bg-gradient-to-br from-indigo-50 to-violet-50' : 'bg-slate-800' }}">
                    ...
                    <div class="flex flex-1 flex-col gap-3 p-5">
                        ...
                        <div class="flex items-start justify-between gap-2">
                            <div class="min-w-0">
                                <h2 class="truncate font-bold text-slate-100">{{ $theme->name }}</h2>
                        ...
                        @if($theme->description)
                            <p class="line-clamp-2 text-sm text-slate-400">{{ $theme->description }}</p>
                        @endif

                        {{-- Stats row --}}
                        <div class="flex flex-wrap items-center gap-x-3 gap-y-1 border-t border-slate-700/50 pt-3 text-xs text-slate-400">
        ...
        {{-- How it works --}}
        <details class="mt-8 rounded-2xl border border-slate-700 bg-slate-800">
            ...
            <div class="border-t border-slate-700 px-5 py-4 text-sm text-slate-400 space-y-2">
                <p><strong class="text-slate-300">Active theme</strong> — only one theme is active at a time. Activating a theme changes the live site immediately.</p>
                <p><strong class="text-slate-300">Widget areas</strong> — declared in <code class="rounded bg-slate-700 px-1 border border-slate-600 text-xs">theme.json</code> as named zones (e.g. <code class="rounded bg-slate-700 px-1 border border-slate-600 text-xs">footer-col-1</code>). Add widgets to populate those zones on the frontend.</p>
                <p><strong class="text-slate-300">Design tokens</strong> — CSS custom properties (e.g. <code class="rounded bg-slate-700 px-1 border border-slate-600 text-xs">--frontend-gold</code>) declared in the theme schema. Override values via the Customize page. Saved overrides are injected into every page's <code class="rounded bg-slate-700 px-1 border border-slate-600 text-xs">&lt;head&gt;</code> as a <code class="rounded bg-slate-700 px-1 border border-slate-600 text-xs">:root { }</code> block.</p>
                <p><strong class="text-slate-300">Template hierarchy</strong> — if a theme provides <code class="rounded bg-slate-700 px-1 border border-slate-600 text-xs">partials/header.blade.php</code> or <code class="rounded bg-slate-700 px-1 border border-slate-600 text-xs">partials/footer.blade.php</code>, those files replace the Phase 2 defaults. Otherwise the defaults are used automatically.</p>
            </div>
        </details>
