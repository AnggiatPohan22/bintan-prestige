...
<div class="admin-page">
    ...
    <div class="mb-6 flex flex-wrap items-start justify-between gap-4">
        <div>
            <h1 class="text-lg font-extrabold text-slate-900">Themes</h1>
    ...
    @if($themes->isEmpty())
        ...
        <div class="admin-empty-state py-12">
            <div class="mx-auto mb-4 grid h-14 w-14 place-items-center rounded-2xl bg-indigo-50 text-2xl text-indigo-400">
                <i class="fa-solid fa-palette" aria-hidden="true"></i>
            </div>
            <p class="font-semibold text-slate-700">No themes registered yet</p>
            <p class="mt-1 max-w-xs text-center text-sm text-slate-400">
                Place a <code class="rounded bg-slate-100 px-1 py-0.5 text-xs">theme.json</code> file
                inside each subdirectory of <code class="rounded bg-slate-100 px-1 py-0.5 text-xs">themes/</code>,
        ...
        <div class="mt-8 rounded-2xl border border-slate-200 bg-slate-50 p-6">
            <h2 class="mb-3 text-sm font-bold text-slate-700">Getting Started</h2>
            <ol class="space-y-2 text-sm text-slate-500">
                <li class="flex gap-2">
                    <span class="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-indigo-100 text-xs font-bold text-indigo-600">1</span>
                    Create a subdirectory under <code class="rounded bg-white px-1 py-0.5 text-xs border border-slate-200">themes/your-theme-slug/</code>
                </li>
                <li class="flex gap-2">
                    <span class="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-indigo-100 text-xs font-bold text-indigo-600">2</span>
                    Add a <code class="rounded bg-white px-1 py-0.5 text-xs border border-slate-200">theme.json</code> manifest with <code class="rounded bg-white px-1 py-0.5 text-xs border border-slate-200">name</code>, <code class="rounded bg-white px-1 py-0.5 text-xs border border-slate-200">slug</code>, and optionally <code class="rounded bg-white px-1 py-0.5 text-xs border border-slate-200">widget_areas</code> and <code class="rounded bg-white px-1 py-0.5 text-xs border border-slate-200">customization_schema</code>
                </li>
                <li class="flex gap-2">
                    <span class="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-indigo-100 text-xs font-bold text-indigo-600">3</span>
                    Click <strong>Scan for Themes</strong> above — the theme will appear in this list
                </li>
                <li class="flex gap-2">
                    <span class="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-indigo-100 text-xs font-bold text-indigo-600">4</span>
    ...
    @else
        ...
        <div class="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
            @foreach($themes as $theme)
                ...
                <div class="flex flex-col rounded-2xl border {{ $theme->is_active ? 'border-indigo-300 ring-1 ring-indigo-200' : 'border-slate-200' }} bg-white shadow-sm">

                    {{-- Screenshot / placeholder --}}
                    <div class="relative flex h-36 items-center justify-center rounded-t-2xl overflow-hidden {{ $theme->is_active ? 'bg-gradient-to-br from-indigo-50 to-violet-50' : 'bg-slate-50' }}">
                        @if($theme->hasScreenshot())
                            ...
                            >
                        @else
                            <span class="select-none text-5xl font-black tracking-tighter {{ $theme->is_active ? 'text-indigo-200' : 'text-slate-200' }}">
                    ...
                    <div class="flex flex-1 flex-col gap-3 p-5">
                        ...
                        <div class="flex items-start justify-between gap-2">
                            <div class="min-w-0">
                                <h2 class="truncate font-bold text-slate-900">{{ $theme->name }}</h2>
                        ...
                        @if($theme->description)
                            <p class="line-clamp-2 text-sm text-slate-500">{{ $theme->description }}</p>
                        @endif

                        {{-- Stats row --}}
                        <div class="flex flex-wrap items-center gap-x-3 gap-y-1 border-t border-slate-100 pt-3 text-xs text-slate-400">
        ...
        {{-- How it works --}}
        <details class="mt-8 rounded-2xl border border-slate-200 bg-slate-50">
            ...
            <div class="border-t border-slate-200 px-5 py-4 text-sm text-slate-500 space-y-2">
                <p><strong class="text-slate-700">Active theme</strong> — only one theme is active at a time. Activating a theme changes the live site immediately.</p>
                <p><strong class="text-slate-700">Widget areas</strong> — declared in <code class="rounded bg-white px-1 border border-slate-200 text-xs">theme.json</code> as named zones (e.g. <code class="rounded bg-white px-1 border border-slate-200 text-xs">footer-col-1</code>). Add widgets to populate those zones on the frontend.</p>
                <p><strong class="text-slate-700">Design tokens</strong> — CSS custom properties (e.g. <code class="rounded bg-white px-1 border border-slate-200 text-xs">--frontend-gold</code>) declared in the theme schema. Override values via the Customize page. Saved overrides are injected into every page's <code class="rounded bg-white px-1 border border-slate-200 text-xs">&lt;head&gt;</code> as a <code class="rounded bg-white px-1 border border-slate-200 text-xs">:root { }</code> block.</p>
                <p><strong class="text-slate-700">Template hierarchy</strong> — if a theme provides <code class="rounded bg-white px-1 border border-slate-200 text-xs">partials/header.blade.php</code> or <code class="rounded bg-white px-1 border border-slate-200 text-xs">partials/footer.blade.php</code>, those files replace the Phase 2 defaults. Otherwise the defaults are used automatically.</p>
            </div>
        </details>
