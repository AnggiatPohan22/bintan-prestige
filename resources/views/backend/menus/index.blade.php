...
<div class="admin-page">
    <div class="mb-6">
        <h1 class="text-lg font-extrabold text-slate-100">Menus</h1>
    ...
    @if($menus->isEmpty())
        <div class="admin-empty-state py-10">
            <p class="font-medium text-slate-400">No menus found.</p>
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            @foreach($menus as $menu)
                ...
                <a href="{{ route('admin.menus.edit', $menu) }}"
                   class="group flex flex-col rounded-2xl border border-slate-700 bg-slate-800 p-5 transition hover:-translate-y-0.5 hover:border-violet-500/50">
                    <div class="flex items-start justify-between">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <h2 class="font-bold text-slate-100">{{ $meta['title'] }}</h2>
                    ...
                    <div class="mt-4 min-h-[2.5rem] flex flex-wrap gap-1.5">
                        @forelse($preview as $item)
                            <span class="rounded-full bg-slate-800 px-2.5 py-1 text-xs font-medium text-slate-400">{{ $item->label }}</span>
                        @empty
