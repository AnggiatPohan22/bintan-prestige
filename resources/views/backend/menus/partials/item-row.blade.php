...
<div class="flex items-center gap-2 {{ $isChild ? 'px-2.5 py-1.5' : 'px-3 py-2.5' }}">
    ...
    <button type="button" data-handle
            class="cursor-grab text-slate-300 hover:text-slate-400 active:cursor-grabbing" title="Drag to reorder">
        <i class="fa-solid fa-grip-vertical"></i>
    </button>

    {{-- Type icon --}}
    <span class="grid h-7 w-7 shrink-0 place-items-center rounded-lg bg-slate-800 text-slate-400">
        <i class="fa-solid {{ $meta['icon'] }} text-xs"></i>
    </span>

    {{-- Label + resolved URL --}}
    <div class="min-w-0 flex-1">
        <p class="truncate text-sm font-semibold text-slate-300">
            {{ $item->label }}
            @unless($item->is_active)
                <span class="ml-1 rounded bg-slate-200 px-1.5 py-0.5 text-[10px] font-medium text-slate-400">Hidden</span>
            @endunless
        </p>
    ...
    <div class="flex shrink-0 items-center gap-1">
        <button type="button"
                ...
                class="grid h-8 w-8 place-items-center rounded-lg text-slate-400 hover:bg-slate-700 hover:text-slate-200" title="Edit">
            <i class="fa-solid fa-pen text-xs"></i>
        </button>

        <form method="POST" action="{{ route('admin.menu-items.toggle-active', [$menu, $item]) }}">
            @csrf
            <button type="submit" class="grid h-8 w-8 place-items-center rounded-lg text-slate-400 hover:bg-slate-700 hover:text-slate-200"
