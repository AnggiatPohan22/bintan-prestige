...
@if($media->isEmpty())
    <div class="admin-empty-state py-12">
        ...
        <p class="font-medium text-slate-400">No media found.</p>
    ...
    <div class="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
        @foreach($media as $item)
            <button type="button"
                    ...
                    class="group overflow-hidden rounded-xl border border-slate-700 bg-slate-800 text-left transition hover:-translate-y-0.5 hover:border-violet-500/50">
                ...
                <div class="px-3 py-2">
                    <p class="truncate text-xs font-semibold text-slate-300" title="{{ $item->original_name }}">{{ $item->original_name }}</p>
                    <p class="mt-0.5 flex items-center justify-between text-[11px] text-slate-400">
                        <span>{{ $item->size_for_humans }}</span>
                        <span class="rounded bg-slate-800 px-1.5 font-mono uppercase">{{ $item->extension }}</span>
                    </p>
