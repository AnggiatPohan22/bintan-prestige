...
<div class="admin-page"
    ...
    <div class="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h1 class="text-lg font-extrabold text-slate-100">Media Library</h1>
            <p class="text-sm text-slate-400">Upload, search, and reuse images across the site.</p>
        </div>
    ...
    <div class="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div class="flex flex-wrap gap-1.5">
            <a href="{{ route('admin.media.index', ['search' => $search]) }}"
               class="rounded-full px-3 py-1.5 text-xs font-semibold {{ ! $activeType ? 'bg-indigo-600 text-white' : 'bg-slate-800 text-slate-400 hover:bg-slate-200' }}">
                All
            </a>
            @foreach($extensions as $ext)
                <a href="{{ route('admin.media.index', ['type' => $ext, 'search' => $search]) }}"
                   class="rounded-full px-3 py-1.5 text-xs font-semibold uppercase {{ $activeType === $ext ? 'bg-indigo-600 text-white' : 'bg-slate-800 text-slate-400 hover:bg-slate-200' }}">
                    {{ $ext }}
                </a>
            @endforeach
        </div>
    ...
    <div x-show="detailOpen" x-cloak class="fixed inset-0 z-[55]" style="display:none">
        <div class="absolute inset-0 bg-slate-900/40" x-on:click="closeDetail()"
        ...
        <div class="absolute inset-y-0 right-0 flex w-full max-w-md flex-col bg-slate-900 shadow-2xl"
            ...
            <div class="flex items-center justify-between border-b border-slate-700/50 px-5 py-4">
                <h2 class="font-bold text-slate-100">Media details</h2>
                <button type="button" x-on:click="closeDetail()" class="grid h-8 w-8 place-items-center rounded-full text-slate-400 hover:bg-slate-700 hover:text-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500" aria-label="Close details">
            ...
            <div class="flex-1 space-y-5 overflow-y-auto px-5 py-5">
                ...
                {{-- Meta --}}
                <dl class="space-y-1.5 rounded-xl bg-slate-800 p-4 text-xs">
                    <div class="flex justify-between"><dt class="text-slate-400">File</dt><dd class="font-medium text-slate-300 truncate pl-3" x-text="selected.name"></dd></div>
                    <div class="flex justify-between"><dt class="text-slate-400">Type</dt><dd class="font-medium text-slate-300" x-text="selected.ext"></dd></div>
                    <div class="flex justify-between"><dt class="text-slate-400">Size</dt><dd class="font-medium text-slate-300" x-text="selected.size"></dd></div>
                    <div class="flex justify-between"><dt class="text-slate-400">Dimensions</dt><dd class="font-medium text-slate-300" x-text="selected.dimensions"></dd></div>
                    <div class="flex justify-between"><dt class="text-slate-400">Uploaded by</dt><dd class="font-medium text-slate-300" x-text="selected.uploader"></dd></div>
                    <div class="flex justify-between"><dt class="text-slate-400">Date</dt><dd class="font-medium text-slate-300" x-text="selected.date"></dd></div>
                </dl>
