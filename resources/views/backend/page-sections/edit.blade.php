...
<div class="admin-page">
    ...
    <div class="admin-form-card">
        <form method="POST" action="{{ route('admin.page-sections.update', $pageSection) }}" enctype="multipart/form-data" class="space-y-6">
            ...
            <div><label class="admin-form-label">Page key</label><input type="text" value="{{ $pageSection->page_key }}" disabled class="admin-input bg-slate-800"></div>
            <div><label class="admin-form-label">Section key</label><input type="text" value="{{ $pageSection->section_key }}" disabled class="admin-input bg-slate-800"></div>
            ...
            @if(count($mediaSlots))
                <div class="md:col-span-2">
                    <h2 class="text-lg font-bold text-slate-100">Section image slots</h2>
                    <p class="mt-1 text-sm text-slate-400">Upload sesuai posisi gambar di layout section ini.</p>
                    ...
                    <div class="mt-4 grid grid-cols-1 gap-4 md:grid-cols-2">
                        @foreach($mediaSlots as $slot)
                            ...
                            <div class="rounded-xl border border-slate-200 bg-slate-800 p-4">
                                ...
                                <p class="mt-2 text-xs text-slate-400">{{ $slot['hint'] ?? 'Leave empty to keep current image.' }}</p>
                                @if($slotMedia?->url)
                                ...
                                @if($supportsMediaDisplayOptions ?? false)
                                    <div class="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2">
                                        <div>
                                            ...
                                            <p class="mt-2 text-xs text-slate-400">Controls how the image fills its frame.</p>
                                        </div>
                                        <div>
                                            ...
                                            <p class="mt-2 text-xs text-slate-400">Controls which part stays visible when cropped.</p>
                                        </div>
                                    </div>
                                @endif
            ...
            @else
                <div class="md:col-span-2 rounded-xl border border-slate-200 bg-slate-800 p-4">
                    <h2 class="text-sm font-bold text-slate-100">No section image upload for this layout</h2>
                    <p class="mt-2 text-sm text-slate-400">Frontend for this section does not render a dedicated page-section image. It may use product, category, testimonial avatar, footer, or global placeholder media instead.</p>
                </div>
            @endif
            ...
            <label class="flex items-center gap-3"><input type="checkbox" name="is_active" value="1" @checked(old('is_active', $pageSection->is_active))><span class="text-sm font-semibold text-slate-300">Active</span></label>
            </div>
    ...
    @if($pageSection->media->count())
        <div class="admin-card">
            <div class="admin-card-header">
                <h2 class="text-lg font-extrabold text-slate-100">Stored Section Media</h2>
            </div>
