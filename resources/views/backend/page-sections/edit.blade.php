...
<div class="rounded-xl bg-white p-6 shadow">
    <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
            ...
            <p class="mt-1 text-sm text-slate-500">Upload image, mobile image, or up to 10 gallery images for reusable sections.</p>
        </div>
    ...
    <form method="POST" action="{{ route('admin.page-sections.update', $pageSection) }}" enctype="multipart/form-data" class="space-y-5">
        ...
        <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
            ...
            <div>
                ...
                <p class="mt-2 text-xs text-slate-400">Recommended: WebP/JPG max 2MB. Leave empty to keep current image.</p>
            ...
            <div><label class="form-label">Image path</label><input type="text" name="image_path" value="{{ old('image_path', $pageSection->image) }}" class="{{ $inputClass }}"></div>
            <div><label class="form-label">Mobile image path</label><input type="text" name="mobile_image_path" value="{{ old('mobile_image_path', $pageSection->mobile_image) }}" class="{{ $inputClass }}"></div>
    ...
    @if($pageSection->media->count())
        <div class="mt-8 border-t pt-6">
            <h2 class="text-lg font-bold text-slate-800">Section Gallery</h2>
                ...
                @foreach($pageSection->media as $media)
                    <div class="rounded-xl border bg-white p-3 shadow-sm">
                        ...
                        <form method="POST" action="{{ route('admin.page-sections.media.destroy', $media) }}" class="mt-3">
                            @csrf
                            @method('DELETE')
                            <button type="submit" onclick="return confirm('Delete this section image?')" class="w-full rounded-lg border border-red-200 px-3 py-2 text-xs font-semibold text-red-600 hover:bg-red-50">Delete Image</button>
                        </form>
                    </div>
                @endforeach
            </div>
