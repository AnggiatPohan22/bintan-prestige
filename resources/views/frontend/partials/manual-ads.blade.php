        ...
        <div class="bp-manual-ads__inner">
            ...
            <div class="bp-manual-ads__content">
                <span class="bp-manual-ads__label">
                    {{ $section->label ?? 'Enjoy Summer Deals' }}
                </span>

                <h2 id="manual-ads-title" class="bp-manual-ads__title title-section">
                    {{ $section->title ?? 'Up to 40% Discount!' }}
                </h2>
            ...
            <div class="bp-manual-ads__visual">
                @if($image)
                    <img src="{{ \Illuminate\Support\Str::startsWith($image, ['http://', 'https://', '/']) ? $image : asset('storage/' . $image) }}" alt="{{ $section->title ?? 'Bintan Prestige promotion' }}" loading="lazy" decoding="async">
                @else
                ...
                <div class="bp-manual-ads__overlay-title">
                    {{ $extraData['overlay_title'] ?? "Let's Discover The Whole World!" }}
                </div>
            </div>
        </div>
    </div>
