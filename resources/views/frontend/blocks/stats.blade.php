...
<section class="px-6 py-12 sm:py-16" style="{{ $bgStyle }}" @if($heading !== '') aria-labelledby="stats-heading-{{ $block->id }}" @else aria-label="Key statistics" @endif>
    <div class="mx-auto max-w-7xl">
        @if($heading !== '')
            <h2 id="stats-heading-{{ $block->id }}" class="text-3xl font-bold tracking-tight text-slate-900 {{ $alignment }}">{{ $heading }}</h2>
        @endif
        @if($items->isNotEmpty())
            <dl class="mt-8 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
                @foreach($items as $item)
                    <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm {{ $alignment }}">
                        @if(! empty($item['value']))<dd class="text-4xl font-bold text-[var(--frontend-gold,#D4AF37)]">{{ $item['value'] }}</dd>@endif
                        @if(! empty($item['label']))<dt class="mt-2 font-semibold text-slate-900">{{ $item['label'] }}</dt>@endif
