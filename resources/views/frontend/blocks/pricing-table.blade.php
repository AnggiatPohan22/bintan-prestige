...
<section class="px-6 py-12 sm:py-16" style="{{ $bgStyle }}" @if($heading !== '') aria-labelledby="pricing-heading-{{ $block->id }}" @else aria-label="Pricing plans" @endif>
    <div class="mx-auto max-w-7xl">
        @if($heading !== '')<h2 id="pricing-heading-{{ $block->id }}" class="text-center text-3xl font-bold tracking-tight sm:text-4xl">{{ $heading }}</h2>@endif
        ...
        @if($plans->isNotEmpty())
            <div class="mt-10 grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
                @foreach($plans as $plan)
                    ...
                    <article class="flex flex-col rounded-2xl border bg-white p-6 shadow-sm {{ $featured ? 'border-[var(--frontend-gold,#c8a24a)] ring-2 ring-[var(--frontend-gold,#c8a24a)]' : 'border-slate-200' }}">
                        @if($featured)<p class="mb-3 text-xs font-bold uppercase tracking-widest text-[var(--frontend-gold-dark,#b08735)]">Featured</p>@endif
                        @if(! empty($plan['name']))<h3 class="text-2xl font-bold">{{ $plan['name'] }}</h3>@endif
                        @if(! empty($plan['price']))
                        ...
                        @if($features)
                            <ul class="mt-6 flex-1 space-y-3 text-sm text-slate-700">
                                @foreach($features as $feature)<li class="flex gap-2"><span class="text-[var(--frontend-gold-dark,#b08735)]" aria-hidden="true">✓</span><span>{{ $feature }}</span></li>@endforeach
                            </ul>
                        @endif
                        @if(! empty($plan['button_text']) && ! empty($plan['button_url']))
                            <a href="{{ $plan['button_url'] }}" class="mt-8 inline-flex min-h-11 items-center justify-center rounded-full bg-[var(--frontend-gold,#c8a24a)] px-6 py-3 text-sm font-bold text-[var(--frontend-black,#090806)] transition hover:opacity-90 focus:outline-none focus:ring-2 focus:ring-[var(--frontend-gold,#c8a24a)] focus:ring-offset-2">{{ $plan['button_text'] }}</a>
                        @endif
                    </article>
                @endforeach
            </div>
