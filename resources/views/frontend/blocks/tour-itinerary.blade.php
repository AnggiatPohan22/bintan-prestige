...
<section class="px-6 py-12 sm:py-16" style="{{ $bgStyle }}" @if($heading !== '') aria-labelledby="itinerary-heading-{{ $block->id }}" @else aria-label="Tour itinerary" @endif>
    <div class="mx-auto max-w-4xl">
        @if($heading !== '')<h2 id="itinerary-heading-{{ $block->id }}" class="text-3xl font-bold tracking-tight text-slate-900 sm:text-4xl">{{ $heading }}</h2>@endif
        @if($intro !== '')<p class="mt-4 max-w-3xl text-lg text-slate-600">{{ $intro }}</p>@endif
        @if($items->isNotEmpty())
            <ol class="relative mt-10 border-l-2 border-[var(--frontend-gold,#D4AF37)] pl-8">
                @foreach($items as $item)
                    <li class="relative pb-10 last:pb-0">
                        <span class="absolute -left-[2.6rem] top-1 h-5 w-5 rounded-full border-4 border-white bg-[var(--frontend-gold,#D4AF37)] shadow" aria-hidden="true"></span>
                        @if(! empty($item['marker']))<p class="text-xs font-bold uppercase tracking-widest text-[var(--frontend-gold,#B28B22)]">{{ $item['marker'] }}</p>@endif
                        @if(! empty($item['title']))<h3 class="mt-1 text-xl font-bold text-slate-900">{{ $item['title'] }}</h3>@endif
